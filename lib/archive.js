...
class Archive {
  ...
  readIndex( callback ) {
    ...
    this.readBytes( position, length, ( error, buffer ) => {
      ...
      // Chain of lengths of a block of JSON. Not going into the details of how useless this is.
      // This wasted space could've been used for a "magic", and a version instead.
  ...
  open( filename, options, callback ) {
    ...
    var tasks = [
      ( next ) => {
        fs.open( filename, options.flags || 'r', options, ( error, fd ) => {
          this.fd = fd || null
          next( error )
        })
      },
